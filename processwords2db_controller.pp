unit ProcessWords2Db_controller;
(*
...
type
  ...
  published
    ...
    property DMod : TWord_Dmod read FDmod write FDmod;
  end;
...
begin
  ...
  try
    ...
    try
      sLst4Words.LoadFromStream(strm);
    finally
      strm.Free;
    end;
...
begin
  ...
  try
    ...
    try
      ...
      on E : EWordsDbException do
      begin
        tx.Rollback;
        ShowMessage(Concat('cleanWordsTable: database open has failed.', sLineBreak, E.Message));
        Abort;
			end;
		end;
  finally
...
var
  ...
  qry : TSQLQuery;
begin
  ShowMessage('TFrmPsWords2DBController.ActWrtWords2WordscapeExecute is not fully implemented.');
  Exit;

  if not cleanWordsTable then
    Exit;
  pgBar.Min := 0;
  pgBar.Max := sLst4Words.Count;
  rows2Ps := sLst4Words.Count;
  ...
  try
    ...
    qry.Prepare;            // this can't be prepared, it has no SQL
    pgBar.Visible := True;
    tx.StartTransaction;
    try
      while i < sLst4Words.Count - 1 do
      begin
        //if RegEx.IsMatch(sLst4Words.Strings[i]) then
        begin
          sLst4Words.Delete(i);
          Continue;
        end;
...
{$ENDIF}
        // process the line
        qry.Params[0].AsString := sLst4Words.Strings[i];
        qry.ExecSQL;
        if i mod 50 = 0 then
        begin
...
begin
  ...
  sLst4Words := TStringList.Create;
  FDmod := TWord_Dmod.Create(self);
